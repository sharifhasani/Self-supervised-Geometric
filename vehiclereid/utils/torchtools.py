...
def resume_from_checkpoint(ckpt_path, model, optimizer=None, use_apex=False):
    ...
    print('** previous epoch = {}\t previous mAP_i2t = {:.1%}, mAP_i2i = {:.1%}, rank1 = {:.1%}'
          .format(start_epoch, ckpt['mAP_i2t'], ckpt['mAP_i2i'], ckpt['rank1']))
    return start_epoch
...
def load_pretrained_weights(model, weight_path):
    ...
    if len(matched_layers) == 0:
        assert ValueError('The pretrained weights "{}" cannot be loaded, please check the key names manually (** ignored and continue **)'.format(
                weight_path))
    else:
