...
def eval_building(distmat, q_pids, g_pids, q_camids, g_camids, max_rank):
    """Evaluation with veri metric
    Key: for each query identity, its gallery images from the same camera view are discarded.
    """
    num_q, num_g = distmat.shape
...
def evaluate(name, distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50):
    if name == 'veri':
        return eval_veri(distmat, q_pids, g_pids, q_camids, g_camids, max_rank)
    if name == 'building':
        return eval_building(distmat, q_pids, g_pids, q_camids, g_camids, max_rank)
    elif name == 'vehicleID':
        return eval_vehicleid(distmat, q_pids, g_pids, q_camids, g_camids, max_rank)
    elif name == 'market1501' or name == 'dukemtmc':
        return eval_market1501(distmat, q_pids, g_pids, q_camids, g_camids, max_rank)
    else:
        raise ValueError('The target dataset {} is not supported!'.format(name))
